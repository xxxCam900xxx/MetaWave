...
- Am Ende eine konsolidierte `metadata.json` aus allen vorhandenen `.info.json` Dateien erzeugen, die sich – wenn möglich – an der aktuellen Playlist orientiert.
- Robustheit: Auch bei fehlerhaften Chunks sollen vorhandene Daten nutzbar bleiben.
- Idempotenz: Bereits heruntergeladene Videos werden beim Flatten und beim Download übersprungen, so dass Wiederholungs-Läufe nur neue Inhalte verarbeiten.
...
`build_metadata` scannt alle `.info.json` Dateien im `SONGS_DIR` und erzeugt daraus `metadata.json`. Wenn möglich, richtet sich die Reihenfolge und Auswahl an der **aktuellen Playlist** aus:

```python
def build_metadata():
    # Alle vorhandenen .info.json Dateien zuerst nach Video-ID indexieren
    info_by_id = {}
    ...
    if playlist_url:
        # Versuch, die aktuelle Playlist zu laden
        ...
        # Metadaten in Playlist-Reihenfolge aufbauen
    ...
    if not used_playlist_filter:
        # Fallback: alle vorhandenen Dateien ungefiltert verwenden
...
- Wenn `PLAYLIST_URL` verfügbar ist und geladen werden kann, enthält `metadata.json` nur Einträge, die auch in der aktuellen Playlist vorkommen **und** für die eine `.info.json` existiert (Playlist-Reihenfolge).
- Wenn die Playlist nicht geladen werden kann, werden alle vorhandenen `.info.json` verwendet (Fallback).
- Dateien, deren IDs nicht mehr in der Playlist sind, bleiben physisch im Volume, erscheinen aber nicht mehr in der neuen `metadata.json`.
...
- Bereits heruntergeladene Videos über vorhandene `.info.json` erkennen und beim Flatten überspringen.
- Nur die **neuen** Video-URLs in `/songs/playlist_urls.json` schreiben.
