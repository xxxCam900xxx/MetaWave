...
import { addEmail, removeEmail, sendWaveTokenToEmail } from "./NotificationLogic.js";
...
router.get("/run-job", authMiddleware, async (req, res) => {
  try {
    ...
    res.status(200).json({ status: 200, message: "Email notification job executed (forced)" });
  } catch (err) {
    console.error(err);
